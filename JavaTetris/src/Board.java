...
public class Board extends JPanel implements ActionListener {
	...
	public void paintComponent(Graphics g) {
		...
		for (int i = 0; i < BoardHeight; ++i) {
			for (int j = 0; j < BoardWidth; ++j) {
				...
				if (shape != MyShape.Tetrominoes.NoShape)
					drawSquare(g2D, 0 + j * squareWidth(), boardTop + i * squareHeight(), shape);
			}
		}
		...
		if (curPiece.getShape() != MyShape.Tetrominoes.NoShape) {
			for (int i = 0; i < 6; ++i) {
				...
				drawSquare(g2D, 0 + x * squareWidth(), boardTop + (BoardHeight - y - 1) * squareHeight(),
						curPiece.getShape());
			}
		}
	}
	...
	private void drawSquare(Graphics2D g2D, int x, int y, MyShape.Tetrominoes shape) {
		Color colors[] = { new Color(0, 0, 0), new Color(204, 102, 102), new Color(102, 204, 102),
				...
				new Color(218, 170, 0), new Color(15, 232, 76), new Color(123, 200, 152), new Color(12, 25, 136),
				new Color(154, 232, 76), new Color(100, 200, 150), new Color(70, 125, 136) };
